...
class Discriminative(nn.Module):
    def __init__(self, num_classes, ndf=64):
        ...
        self.core_net = nn.Sequential(
            ...
            WN_Conv2d(num_classes, n_filter_1, 3, 1, 1), nn.LeakyReLU(0.2),
        ...
        self.out_net = WN_Linear(n_filter_3, 1, train_scale=True, init_stdv=0.1)

    def forward(self, X):
        #if X.dim() == 2:
            #X = X.view(X.size(0), 3, 32, 32)
            #X = X.view(X.size(0), 3, 64, 64)
            #X = X.view(X.size(0), 21, 321, 321)
        
         
        return self.out_net(self.core_net(X)), self.core_net(X)
